...
/**
 * Invoice composer (playbook S111, spec 05 §5.6). Mirrors the quote composer:
 * live line-item editor with totals that recompute as you type (the same
 * computeQuoteTotals the server uses on submit), two-step create->send with a
 * copyable link + WhatsApp share. Money is mono tabular; the server total in
 * the 201 is authoritative.
 */
...
export function InvoiceComposerButton() {
  ...
  return (
    <>
      ...
      {open && <InvoiceComposer open={open} onOpenChange={setOpen} />}
    </>
  )
}
...
function InvoiceComposer({
  ...
  const [sent, setSent] = React.useState<{ link: string; number: string } | null>(null)
  ...
  const create = async () => {
    ...
    try {
      ...
      if (!createRes.ok || !createJson?.invoice?.id) {
        setError('Couldn’t create the invoice. Check the fields and try again.')
        return
      }
      const sendRes = await fetch(`/api/invoices/${createJson.invoice.id}/send`, { method: 'POST' })
      const sendJson = (await sendRes.json().catch(() => null)) as {
        invoice?: { customer_link: string; invoice_number: string }
      } | null
      if (sendRes.ok && sendJson?.invoice?.customer_link) {
        setSent({ link: sendJson.invoice.customer_link, number: sendJson.invoice.invoice_number })
        router.refresh()
      } else {
        setError('Invoice saved as a draft, but sending failed. Open it from the list to retry.')
        router.refresh()
      }
    } catch {
  ...
  if (sent) {
    const waHref = customerPhone.trim()
      ? `https://wa.me/${customerPhone.replace(/\D/g, '')}?text=${encodeURIComponent(
          `Hi ${customerName}, here's your invoice ${sent.number}: ${sent.link}`
        )}`
      : undefined
    return (
      <Modal open={open} onOpenChange={onOpenChange} title="Invoice sent">
        <div className="flex flex-col gap-4">
          <Banner kind="success" text={`Invoice ${sent.number} is ready to share.`} />
    ...
    >
      <div className="flex flex-col gap-4">
              ...
              <div key={index} className="flex items-start gap-2">
                <div className="flex-1">
                  <Input
                    ...
                    placeholder="Bathroom refit — labour"
        ...
        <Textarea
          ...
          placeholder="Payment by bank transfer or WiPay."
          rows={2}
          value={notes}
